...
class EntityRegistryAccessor {
    ...
    fun <T : Entity, U : T> inject(id: String, entityType: EntityType<T>, builder: EntityType.EntityFactory<U>) {
        ...
        val entityTypeBuilder: EntityType.Builder<Entity> = EntityType.Builder.of(builder, MobCategory.MISC)
        ...
        DevUtil.enumerateFields(mappedRegistryClass)

        val frozen = mappedRegistryClass.getDeclaredField("l")
        frozen.isAccessible = true
        frozen.set(registry, false)

        val registering = EntityType::class.java.getDeclaredMethod("a", String::class.java, EntityType.Builder::class.java)
        registering.isAccessible = true
        registering.invoke(null, id, entityTypeBuilder)
